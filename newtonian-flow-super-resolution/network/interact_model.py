...
def real_to_real_traj_fn_newt3d(vel_phys, vel2vort_fn, vort2vel_fn_t, traj_fn):
...
def vel2vort_newt(vel_phys, KMM_vel2vort_fn):
...
def vort2vel_newt(vor_rft, KMM_vort2vel_fn):
...
def coarse_pool_trajectory(omega_traj, pool_d1, pool_d2, pool_d3):
  _, Nx, Ny, Nz, _ = omega_traj.shape
  assert Nx % pool_d1 == 0
  assert Ny % pool_d2 == 0
  assert Nz % pool_d3 == 0
  ...
  omega_pooled_traj = omega_traj[:, ::coarse_x, ::coarse_y,  ::coarse_z, :]
  return omega_pooled_traj
