...
def real_to_real_traj_fn_stf3d(vel_phys, vel2vort_fn, vort2vel_fn_t, traj_fn):
  #vel_phys[x,y,z,C]
  vort_rft = vel2vort_fn(vel_phys) #vort_rft(C)[x,y,z]

  _, traj_rft = traj_fn(vort_rft) #traj_rft(C)[t,x,y,z]
...
def vel2vort(vel_phys, KMM_vel2vort_fn):
  # vel_traj_phys[x,y,z,C]

  r_rft = jnp.fft.rfftn(vel_phys[...,-1],axes=(-3,-2,-1))
  vort_rft = KMM_vel2vort_fn(vel_phys[...,0],vel_phys[...,1],vel_phys[...,2],) #(C)[ix,iy,iz]
  vort_rft = vort_rft + (r_rft,) #(C)[ix,iy,iz]

  return vort_rft


def vort2vel(vor_rft, KMM_vort2vel_fn):
  # vor_traj_phys(C)[x,y,z]

  r_rft = vor_rft[-1] #[x,y,z]
  vel_rft = KMM_vort2vel_fn(vor_rft[0],vor_rft[1],vor_rft[2],vor_rft[3],vor_rft[4],)
  vel_rft = vel_rft[:3] + (r_rft,) #(C)[x,y,z]

  vel_rft = jnp.stack(vel_rft, axis=0) # [C,x,y,z]
  vel_phy = jnp.fft.irfftn(vel_rft, axes=(-3,-2,-1))
  vel_phy = jnp.moveaxis(vel_phy, 0, -1)# [x,y,z,C]
